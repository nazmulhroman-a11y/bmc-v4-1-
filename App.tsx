...
import { BMCData, AnalysisResult, AppState, HistoryItem, BudgetPlan, DepartmentPlan } from './types';
...
import { analyzeBMC, generateBudgetPlan, generateCashFlowAnalysis, generateLaunchData } from './services/geminiService';
import { LayoutGrid, Loader2, History, ArrowLeft, FileText, PlusCircle, HelpCircle } from 'lucide-react';
...
function App() {
  ...
  const handleGenerateCashFlow = async () => {
    ...
    try {
      ...
      const cashFlowData = await generateCashFlowAnalysis(data, analysisResult.budgetPlan);
      const updatedResult = { ...analysisResult, cashFlowAnalysis: cashFlowData };
      setAnalysisResult(updatedResult);
      setAppState(AppState.CASH_FLOW_MANAGER);
    } catch (err) {
      console.error(err);
  ...
  const handleViewLaunchDashboard = async () => {
     ...
     try {
        ...
        console.error(err);
        alert("লঞ্চ ড্যাশবোর্ড জেনারেট করতে সমস্যা হয়েছে। দয়া করে আবার চেষ্টা করুন।");
     } finally {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <header className="bg-white border-b border-slate-200 shadow-sm transition-all sticky top-0 z-50">
        <div className="w-full max-w-screen-2xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setAppState(AppState.INPUT)}>
            ...
            <h1 className="text-lg md:text-2xl font-bold text-slate-800 tracking-tight flex flex-col md:block leading-tight">
              ...
              <span className="text-[10px] text-slate-400 md:hidden font-normal block">Your Business Consultant</span>
            </h1>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-3">
             ...
             >
               ...
               <span className="bg-slate-300 text-slate-700 text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[20px] text-center">{history.length}</span>
             </button>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col relative">
        ...
        {appState === AppState.ANALYZING && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center min-h-[60vh] animate-fade-in">
            ...
            <p className="text-slate-500 max-w-xl text-lg md:text-xl leading-relaxed">
              AI আপনার বিজনেস মডেলের প্রতিটি দিক যাচাই করছে। আর্থিক পূর্বাভাস, রোডম্যাপ ও অ্যাকশন প্ল্যান তৈরি করা হচ্ছে...
            </p>
          </div>
        )}
        ...
        {appState === AppState.LAUNCH_DASHBOARD && analysisResult && analysisResult.launchData && (
           <LaunchDashboard 
        ...
        {appState === AppState.FINANCIAL_PROJECTION && analysisResult && (
          <FinancialProjection result={analysisResult} onBack={handleBackToReport} />
        )}
        ...
        {appState === AppState.BUDGET_PLAN && analysisResult && analysisResult.budgetPlan && (
           <BudgetPlanView 
        ...
        {appState === AppState.CASH_FLOW_MANAGER && analysisResult && analysisResult.cashFlowAnalysis && (
           <CashFlowManager 
