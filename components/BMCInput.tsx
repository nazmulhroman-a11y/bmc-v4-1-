...
import { Handshake, Activity, Box, Gift, Heart, Truck, Users, CreditCard, DollarSign, Save, X, Briefcase, ChevronDown, Target, Factory, ArrowRight } from 'lucide-react';
...
const GOAL_OPTIONS = {
  ...
  Existing: [
    { value: 'ProblemSolving', label: 'সমস্যা সমাধান (Solve Specific Problem)' },
    { value: 'Expansion', label: 'নতুন দিক নির্দেশনা (Expansion/Diversification)' },
...
export const BMCInput: React.FC<BMCInputProps> = ({ data, onChange, onSubmit, onSaveDraft }) => {
  ...
  const isFormValid = Object.keys(data).filter(k => !['businessStage', 'primaryGoal', 'industry'].includes(k))
      ...
      <div className="max-w-4xl mx-auto mb-12 bg-white rounded-[20px] shadow-lg border border-slate-100 p-6 md:p-8 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="relative group">
            <label className="block text-sm font-semibold text-slate-500 mb-2">১. ব্যবসার ধরণ</label>
          ...
          <div className={`relative group transition-opacity duration-300 ${!data.businessStage ? 'opacity-50 pointer-events-none grayscale' : 'opacity-100'}`}>
            <label className="block text-sm font-semibold text-slate-500 mb-2">২. মূল উদ্দেশ্য</label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              >
                <option value="" disabled>নির্বাচন করুন...</option>
                {data.businessStage === 'New' && GOAL_OPTIONS.New.map(opt => (
                  <option key={opt.value} value={opt.value}>{opt.label}</option>
                ))}
                {data.businessStage === 'Existing' && GOAL_OPTIONS.Existing.map(opt => (
                  <option key={opt.value} value={opt.value}>{opt.label}</option>
                ))}
              </select>
              <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
          ...
          <div className="relative group">
            <label className="block text-sm font-semibold text-slate-500 mb-2">৩. ইন্ডাস্ট্রি / খাত (ঐচ্ছিক)</label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        ...
        {data.businessStage && data.primaryGoal && (
           <div className="mt-6 flex items-center gap-3 p-3 bg-indigo-50 border border-indigo-100 rounded-lg animate-fade-in">
              ...
              <p className="text-sm text-indigo-800 font-medium">
                 AI আপনার <span className="font-bold">"{data.businessStage === 'New' ? 'নতুন ব্যবসা' : 'চলমান ব্যবসা'}"</span>-র জন্য 
                 <span className="font-bold mx-1">"{
                    data.businessStage === 'New' 
                      ? GOAL_OPTIONS.New.find(g => g.value === data.primaryGoal)?.label.split('(')[0]
                      : GOAL_OPTIONS.Existing.find(g => g.value === data.primaryGoal)?.label.split('(')[0]
                 }"</span> 
