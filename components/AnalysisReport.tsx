...
import { CheckCircle, AlertTriangle, Lightbulb, Target, ArrowLeft, FileText, Printer, Download, Layers, TrendingUp, ShieldAlert, Megaphone, Quote, CheckSquare, Zap, Activity, Info, CalendarClock, Sparkles, Globe, Users, ShoppingBag, Sword } from 'lucide-react';
...
export const AnalysisReport: React.FC<AnalysisReportProps> = ({ result, data, onReset, onViewActionPlan, onViewProjection, onViewRoadmap }) => {
  ...
  const getReportHTML = () => {
        ...
        marketHtml = `
          <div style="page-break-inside: avoid;">
             ...
             <div style="margin-bottom: 20px; border: 1px solid #ddd; padding: 15px; border-radius: 8px;">
                ...
                <p><strong>TAM (Total Addressable Market):</strong> ${result.marketAnalysis.marketSize.tam}</p>
                <p><strong>SAM (Serviceable Available Market):</strong> ${result.marketAnalysis.marketSize.sam}</p>
                <p><strong>SOM (Serviceable Obtainable Market):</strong> ${result.marketAnalysis.marketSize.som}</p>
                <p><em>Unit: ${result.marketAnalysis.marketSize.unit}, Growth: ${result.marketAnalysis.marketSize.growthRate}</em></p>
             </div>
             ...
             <table style="width: 100%; border-collapse: collapse;">
               ...
               <tbody>
                 ${result.marketAnalysis.competitors.map(c => `
                   <tr>
             ...
             <p style="margin-top: 10px;"><strong>আমাদের USP:</strong> ${result.marketAnalysis.competitiveAdvantage}</p>
          </div>
        `;
    }
    ...
    return `
      <div>
          ...
          <p><strong>স্কোর:</strong> <span class="score" style="color: ${SCORE_COLOR}">${result.overallScore}/100</span></p>
  ...
  const handleDownloadPDF = (includeInput: boolean) => {
        ...
        contentHTML += `
          <div class="pdf-card">
             ...
             <div style="margin-bottom: 15px; background: #f0f9ff; padding: 10px; border-radius: 8px; border: 1px solid #bae6fd;">
                <strong>TAM:</strong> ${result.marketAnalysis.marketSize.tam}<br/>
                <strong>SAM:</strong> ${result.marketAnalysis.marketSize.sam}<br/>
             ...
             <table>
               ...
               <tbody>
                 ${result.marketAnalysis.competitors.map(c => `<tr><td><b>${c.name}</b><br/><small>${c.type}</small></td><td>${c.strength}</td><td>${c.weakness}</td></tr>`).join('')}
               </tbody>
             </table>
             <div style="margin-top: 10px;"><b>USP:</b> ${result.marketAnalysis.competitiveAdvantage}</div>
          </div>
        `;
    }
    ...
    contentHTML += `
      <div class="pdf-card">
        ...
        <table>
          ...
          <tbody>
            ${result.segmentAnalysis.map(s => `<tr><td><b>${s.segment}</b></td><td>${s.feedback}</td><td>${s.score}/10</td></tr>`).join('')}
          </tbody>
        </table>
      </div>
    `;
      ...
      contentHTML += `
        <div class="pdf-card">
          ...
          <table>
             ...
             <tbody>
               ${result.riskAnalysis.map(r => `<tr><td>${r.risk}</td><td>${r.impact}</td><td>${r.probability}</td><td>${r.mitigation}</td></tr>`).join('')}
             </tbody>
          </table>
        </div>
      `;
    ...
    contentHTML += `
      <div class="pdf-card">
        ...
        <p><strong>Growth Hack:</strong> ${result.marketingStrategy?.growthHack}</p>
        ...
        <div class="pdf-card">
          ...
          ${result.departmentalActionPlan.map(d => `
            <div style="margin-bottom: 15px;">
              <div style="font-weight: bold; color: #2563eb;">${d.department}</div>
              <ul style="margin-top: 5px;">
                 ${d.roles.map(r => `<li><b>${r.role}:</b> ${r.tasks.map(t => t.text).slice(0, 2).join(', ')}...</li>`).join('')}
              </ul>
            </div>
          `).join('')}
        </div>
    ...
    const opt = {
      ...
      html2canvas:  { scale: 2, useCORS: true, letterRendering: true, windowWidth: 800 }, // Fixed window width helps
  ...
  return (
    <div className="w-full max-w-screen-2xl mx-auto p-4 sm:p-6 lg:p-10 space-y-8 animate-fade-in pb-24 font-sans text-slate-800">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-6 no-print bg-white/80 backdrop-blur-md p-5 rounded-2xl shadow-lg border border-white/20 z-30">
        <div>
           <h2 className="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight">রিপোর্ট ড্যাশবোর্ড</h2>
           <p className="text-slate-500 font-medium">আপনার ব্যবসার এআই বিশ্লেষণ</p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          ...
          >
             ...
             অ্যাকশন প্ল্যান
             <span className="bg-emerald-700 text-[10px] px-1.5 rounded-full text-white/90">Interactive</span>
          </button>
          ...
          >
             ...
             রোডম্যাপ
             <span className="bg-purple-700 text-[10px] px-1.5 rounded-full text-white/90 border border-purple-500/50">Budget & Risk</span>
          </button>
          ...
          <div className="flex items-center gap-1 bg-slate-800 p-1 rounded-xl border border-slate-900 shadow-md">
             <span className="text-xs font-bold text-slate-200 px-2 uppercase tracking-wider hidden xl:inline">Input+Report</span>
             <button 
      ...
      <div id="report-content" ref={reportRef} className="space-y-8">
            ...
            <div className="xl:w-1/3 bg-slate-50/50 p-8 xl:p-12 flex flex-col items-center justify-center border-b xl:border-b-0 xl:border-r border-slate-100">
              <div className="w-56 h-56 relative mb-6">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-6xl font-black tracking-tighter" style={{ color: SCORE_COLOR }}>{result.overallScore}</span>
                  <span className="text-sm text-slate-400 font-bold uppercase tracking-widest mt-1">Score</span>
                </div>
              </div>
              
              <div className={`px-4 py-1.5 rounded-full text-sm font-bold border ${result.overallScore >= 80 ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : result.overallScore >= 60 ? 'bg-indigo-100 text-indigo-700 border-indigo-200' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>
                {result.overallScore >= 80 ? 'চমৎকার পরিকল্পনা' : result.overallScore >= 60 ? 'ভালো পরিকল্পনা' : 'উন্নতি প্রয়োজন'}
              </div>
            </div>
             ...
             <section className="bg-white p-8 rounded-[2rem] shadow-lg border border-slate-100 flex flex-col">
                ...
                <div className="flex-1 flex flex-col justify-center items-center relative py-6">
                   ...
                   <div className="relative w-72 h-72 rounded-full border-2 border-sky-100 bg-sky-50 flex items-center justify-center text-center shadow-sm z-10">
                      <div className="absolute top-4 text-xs font-bold text-sky-400 uppercase tracking-widest">Total Market (TAM)</div>
                      ...
                      <div className="relative w-52 h-52 rounded-full border-2 border-blue-200 bg-blue-100 flex items-center justify-center text-center shadow-md z-20">
                         <div className="absolute top-3 text-xs font-bold text-blue-500 uppercase tracking-widest">Serviceable (SAM)</div>
                         ...
                         <div className="relative w-32 h-32 rounded-full border-2 border-indigo-300 bg-indigo-600 flex flex-col items-center justify-center text-center shadow-lg z-30 text-white">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-indigo-200 mb-1">Target (SOM)</span>
                            <span className="font-bold text-sm leading-tight px-2">{result.marketAnalysis.marketSize.som}</span>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="w-full mt-8 grid grid-cols-3 gap-2 text-center text-xs">
                      <div className="p-3 bg-sky-50 rounded-xl border border-sky-100">
                         ...
                         <div className="text-slate-600">{result.marketAnalysis.marketSize.tam}</div>
                      </div>
                      <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
                         <div className="font-bold text-blue-600 mb-1">SAM</div>
                         <div className="text-slate-600">{result.marketAnalysis.marketSize.sam}</div>
                      </div>
                      <div className="p-3 bg-indigo-50 rounded-xl border border-indigo-100">
                         <div className="font-bold text-indigo-600 mb-1">Growth</div>
                         <div className="text-slate-600">{result.marketAnalysis.marketSize.growthRate}</div>
                      </div>
                   </div>
                </div>
             </section>
                ...
                <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                      ...
                      <div key={idx} className="p-4 rounded-xl border border-slate-100 bg-slate-50 hover:bg-white hover:shadow-md transition-all">
                         <div className="flex justify-between items-start mb-2">
                            <div>
                               ...
                               <span className="text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full">{comp.type}</span>
                            </div>
                         </div>
                         <div className="grid grid-cols-2 gap-4 text-sm mt-3">
                            <div>
                               <span className="text-xs font-bold text-emerald-600 uppercase block mb-1">Strength</span>
                               <p className="text-slate-600 leading-tight">{comp.strength}</p>
                            </div>
                            <div>
                               <span className="text-xs font-bold text-red-500 uppercase block mb-1">Weakness</span>
                               <p className="text-slate-600 leading-tight">{comp.weakness}</p>
                            </div>
                         </div>
                      </div>
                   ))}
                   ...
                   <div className="mt-6 p-5 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl text-white shadow-lg">
                      <div className="flex items-center gap-2 mb-2">
                         ...
                         <span className="text-xs font-bold uppercase tracking-wider text-indigo-100">Our Competitive Advantage (USP)</span>
                      </div>
          ...
          <section className="bg-white p-8 rounded-[2rem] shadow-lg border border-slate-100 flex flex-col">
             ...
             <div className="space-y-4 overflow-y-auto max-h-[350px] pr-2 custom-scrollbar">
               {result.segmentAnalysis.map((seg, idx) => (
                 <div key={idx} className="group p-4 rounded-2xl hover:bg-slate-50 border border-slate-100 transition-colors">
                   <div className="flex justify-between items-center mb-2">
                     <span className="font-bold text-slate-700">{seg.segment}</span>
                     <div className="flex items-center gap-2">
                        <div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden">
                           <div className={`h-full rounded-full ${seg.score >= 7 ? 'bg-green-500' : seg.score >= 4 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{ width: `${seg.score * 10}%` }}></div>
                        </div>
                        <span className="text-sm font-bold text-slate-600">{seg.score}/10</span>
                     </div>
                   </div>
                   <p className="text-sm text-slate-500 leading-relaxed group-hover:text-slate-700">{seg.feedback}</p>
                 </div>
               ))}
             </div>
          </section>
           ...
           <section className="xl:col-span-2 bg-white p-8 rounded-[2rem] shadow-lg border border-slate-100 flex flex-col">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
                  ...
                  <div className="bg-pink-50 rounded-2xl p-6 flex flex-col justify-center border border-pink-100 relative overflow-hidden group">
                      ...
                      <span className="text-xs font-bold text-pink-400 uppercase tracking-wider mb-2 relative z-10">Tagline</span>
                      <p className="text-2xl font-bold text-pink-700 relative z-10 leading-tight">"{result.marketingStrategy?.tagline || 'N/A'}"</p>
                  </div>
                  ...
                  <div className="bg-purple-50 rounded-2xl p-6 flex flex-col justify-center border border-purple-100 relative overflow-hidden group">
                      ...
                      <span className="text-xs font-bold text-purple-400 uppercase tracking-wider mb-2 relative z-10">Growth Hack</span>
                      <p className="text-lg font-medium text-purple-800 relative z-10">{result.marketingStrategy?.growthHack || 'N/A'}</p>
                  </div>

                  {/* Channels */}
                  <div className="md:col-span-2">
                     <h4 className="font-bold text-slate-700 mb-3 text-sm uppercase tracking-wide">সেরা চ্যানেলসমূহ</h4>
                     <div className="flex flex-wrap gap-2">
                       {(result.marketingStrategy?.topChannels || []).map((ch, i) => (
                         <span key={i} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-600 text-sm font-medium transition-colors cursor-default border border-slate-200">{ch}</span>
                       ))}
                     </div>
                  </div>
              </div>
           ...
           <section className="bg-white p-8 rounded-[2rem] shadow-lg border border-slate-100 flex flex-col">
              ...
              <ul className="space-y-3">
                {(result.kpis || []).map((kpi, i) => (
                  <li key={i} className="flex items-center gap-4 p-3 rounded-xl hover:bg-blue-50 transition-colors group">
                    <span className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center text-sm font-bold text-blue-700 flex-shrink-0 group-hover:bg-blue-200 transition-colors">{i + 1}</span>
                    <span className="text-slate-600 font-medium group-hover:text-blue-900 transition-colors">{kpi}</span>
                  </li>
                ))}
              </ul>
           </section>
                ...
                <table className="w-full min-w-[700px] border-collapse text-left">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      <th className="p-4 pl-6 text-sm font-bold text-slate-500 uppercase tracking-wider">ঝুঁকি (Risk)</th>
                      <th className="p-4 text-sm font-bold text-slate-500 uppercase tracking-wider">প্রভাব</th>
                      <th className="p-4 text-sm font-bold text-slate-500 uppercase tracking-wider">সম্ভাবনা</th>
                      <th className="p-4 text-sm font-bold text-slate-500 uppercase tracking-wider">সমাধান (Mitigation)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {result.riskAnalysis.map((item, i) => (
                      <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                        <td className="p-4 pl-6 font-semibold text-slate-700">{item.risk}</td>
                        <td className="p-4">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${item.impact === 'High' ? 'bg-red-50 text-red-600 border-red-100' : item.impact === 'Medium' ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-green-50 text-green-600 border-green-100'}`}>
                            {item.impact}
                          </span>
                        </td>
                        <td className="p-4">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${item.probability === 'High' ? 'bg-red-50 text-red-600 border-red-100' : item.probability === 'Medium' ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-green-50 text-green-600 border-green-100'}`}>
                            {item.probability}
                          </span>
                        </td>
                        <td className="p-4 text-slate-600 text-sm">{item.mitigation}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <section className="break-inside-avoid">
           <h3 className="text-2xl font-bold text-slate-800 mb-6 pl-2">SWOT অ্যানালাইসিস</h3>
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
             <div className="md:w-1/3">
                ...
                <p className="text-indigo-200 text-lg leading-relaxed">
                   আপনার ব্যবসাকে পরবর্তী ধাপে নিয়ে যেতে আমাদের AI এই বিষয়গুলোতে নজর দেওয়ার পরামর্শ দিচ্ছে।
                </p>
             </div>
             <div className="flex-1 grid grid-cols-1 gap-4">
                {result.suggestions.map((suggestion, index) => (
                  <div key={index} className="flex gap-4 bg-white/10 p-5 rounded-2xl backdrop-blur-md border border-white/5 hover:bg-white/15 transition-colors">
                    <span className="font-bold text-xl text-indigo-300 opacity-60">#{index + 1}</span>
                    <p className="text-lg text-white/90 font-light leading-relaxed">{suggestion}</p>
                  </div>
                ))}
             </div>
          </div>
        ...
        <div className="text-center pt-8 text-slate-400 text-sm font-medium no-print">
          Generated by BMC AI Analyst • {new Date().toLocaleDateString('bn-BD')}
        </div>
      </div>
...
const SwotCard: React.FC<{ title: string; items: string[]; type: 'strength' | 'weakness' | 'opportunity' | 'threat' }> = ({ title, items, type }) => {
  ...
  const Icon = type === 'strength' ? CheckCircle : type === 'weakness' ? AlertTriangle : type === 'opportunity' ? Lightbulb : ShieldAlert;

  return (
    <div className={`bg-white rounded-2xl shadow-sm border ${style.border} overflow-hidden hover:shadow-md transition-shadow`}>
      <div className={`${style.headerBg} p-4 border-b ${style.border} flex items-center gap-3`}>
      ...
      <div className="p-6">
        <ul className="space-y-3">
          {items.map((item, i) => (
            <li key={i} className="text-slate-700 flex gap-3 items-start leading-relaxed text-base">
              <span className={`mt-2 w-1.5 h-1.5 rounded-full ${style.bullet} flex-shrink-0`} />
              {item}
            </li>
          ))}
        </ul>
